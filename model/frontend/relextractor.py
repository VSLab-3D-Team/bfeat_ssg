...
class RelFeatNaiveExtractor(nn.Module):
    def __init__(self, input_dim, geo_dim, out_dim, num_layers=6, use_node_info_enhancement=True):
        super(RelFeatNaiveExtractor, self).__init__()
        self.obj_proj = nn.Linear(input_dim, 512)
        self.geo_proj = nn.Linear(geo_dim, 512)
        
        self.use_node_info_enhancement = use_node_info_enhancement
        
        in_channels = 4 if use_node_info_enhancement else 3
        self.merge_layer = nn.Conv1d(in_channels=in_channels, out_channels=1, kernel_size=5, stride=1, padding="same")
    ...
    def forward(self, x_i: torch.Tensor, x_j: torch.Tensor, geo_feats: torch.Tensor):
        ...
        if self.use_node_info_enhancement:
            node_diff = p_i - p_j
