...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None, way='mul', use_knn=False):
        ...
        :param way: How to apply attention_weights: 'mul' for multiplication, 'add' for addition
        :param use_knn: Whether to use k-nearest neighbors masking (applied through attention_mask)
        :return: Output tensor and attention map
        '''
        ...
        # Apply attention mask - this is where k-nearest neighbors masking is applied
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None, way='mul', use_knn=False, output_attn=False):
        '''
        :param queries: Query tensor (b_s, nq, d_model)
        :param keys: Key tensor (b_s, nk, d_model)
        :param values: Value tensor (b_s, nk, d_model)
        :param attention_mask: Mask over attention values (b_s, h, nq, nk). True indicates masking.
        :param attention_weights: Multiplicative weights for attention values (b_s, h, nq, nk).
        :param way: How to apply attention_weights: 'mul' for multiplication, 'add' for addition
        :param use_knn: Whether to use k-nearest neighbors masking (applied through attention_mask)
        :param output_attn: Whether to output attention weights
        ...
        if self.identity_map_reordering:
            ...
            out, att = self.attention(q_norm, k_norm, v_norm, attention_mask, attention_weights, way, use_knn)
