...
class BFeatVanillaGAT(torch.nn.Module):
    def __init__(
        ...
        hidden_size=512, node_depth=3, edge_depth=1, use_edge:bool=True, 
        k_neighbors=5, lambda_node_init=0.5, lambda_edge_init=0.5, **kwargs,
    ):
        ...
        for _ in range(max(self.node_depth, self.edge_depth)):
            self.gcn_3ds.append(
        ...
        self.lambda_node = nn.Parameter(torch.tensor(lambda_node_init))
        self.lambda_edge = nn.Parameter(torch.tensor(lambda_edge_init))
    ...
    def forward(
        ...
        lambda_node_value = torch.sigmoid(self.lambda_node)
        lambda_edge_value = torch.sigmoid(self.lambda_edge)
        ...
        if obj_center is not None:
            ...
            k_nearest_mask = torch.zeros(1, 1, N_K, N_K).cuda()
            ...
            for i in range(batch_size):
                ...
                for j in range(L):
                    ...
                    if self.k_neighbors < L:
                        _, topk_indices = torch.topk(dist_j, min(self.k_neighbors, L-1), largest=False)
                    else:
                        topk_indices = torch.arange(L).to(dist.device)
                        topk_indices = topk_indices[topk_indices != j]
                    
                    k_nearest_mask[0, 0, count + j, count + idx_i[topk_indices]] = 1
                    k_nearest_mask[0, 0, count + j, count + j] = 1
                
                weights = torch.cat([center_dist, dist[:, :, None]], dim=-1).unsqueeze(0)  # 1 N N 4
            ...
            final_obj_mask = obj_mask * k_nearest_mask
        else:
            final_obj_mask = None
            rel_mask = None
        ...
        for i in range(max(self.node_depth, self.edge_depth)):
            current_obj_feature = obj_feature_3d.clone()
            current_edge_feature = edge_feature_3d.clone()
            
            if i < self.node_depth:
                obj_feature_3d = obj_feature_3d.unsqueeze(0)
                obj_feature_3d = self.self_attn[i](
                    obj_feature_3d, obj_feature_3d, obj_feature_3d, 
                    attention_mask=final_obj_mask, 
            ...
            if i < self.edge_depth and self.edge_attention:
                edge_feature_3d = edge_feature_3d.unsqueeze(0)
                edge_feature_3d = self.self_attn_rel[i](
                    edge_feature_3d, edge_feature_3d, edge_feature_3d, 
                    attention_mask=rel_mask
                )
                edge_feature_3d = edge_feature_3d.squeeze(0)
            
            if i < self.edge_depth:
                updated_obj, updated_edge = self.gcn_3ds[i](
                    obj_feature_3d, edge_feature_3d, edge_index, 
                    weight=attn_weight, istrain=istrain
                )
            elif i < self.node_depth:
                updated_obj, _ = self.gcn_3ds[i](
                    obj_feature_3d, edge_feature_3d, edge_index, 
                    weight=attn_weight, istrain=istrain
                )
                updated_edge = edge_feature_3d
            
            if i < self.node_depth:
                # hᵢ^(l+1) = λ * hᵢ^(l) + (1−λ) * Δhᵢ^(l)
                obj_feature_3d = lambda_node_value * current_obj_feature + (1 - lambda_node_value) * updated_obj
                
                if i < (self.node_depth-1) or self.node_depth==1:
                    obj_feature_3d = F.relu(obj_feature_3d)
                    obj_feature_3d = self.drop_out(obj_feature_3d)
            
            if i < self.edge_depth:
                # hᵢ^(l+1) = λ * hᵢ^(l) + (1−λ) * Δhᵢ^(l)
                edge_feature_3d = lambda_edge_value * current_edge_feature + (1 - lambda_edge_value) * updated_edge
                
                if i < (self.edge_depth-1) or self.edge_depth==1:
