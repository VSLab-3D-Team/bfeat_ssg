...
from utils.model_utils import Gen_Index, rotation_matrix
import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np

class BFeatJJamTongNet(BaseNetwork):
    def __init__(self, config, n_obj_cls, n_rel_cls, device):
        super(BFeatJJamTongNet, self).__init__()
        self.config = config
    ...
    def forward(
        ...
        if is_train:
            return obj_gnn_feats, edge_gnn_feats, obj_pred, rel_pred, trans # obj_t1_feats, obj_t2_feats
        else:
