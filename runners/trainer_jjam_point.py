...
from model.models.model_jjam_point import BFeatJJamTongSGGPointNet
...
class BFeatJjamTongSGGPointTrainer(BaseTrainer):
    def __init__(self, config, device):
        super().__init__(config, device, multi_view_ssl=True)
        
        self.m_config = config.model
        # Model Definitions
        self.model = BFeatJJamTongSGGPointNet(self.config, self.num_obj_class, self.num_rel_class, device).to(device)
