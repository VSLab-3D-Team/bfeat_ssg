...
from model.models.model_direct_gnn import BFeatTripletContrastvieGNN
...
class BFeatTripletContrastiveTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        for e in range(self.t_config.epoch):
            ...
            for idx, (
                ...
                edge_feats, triplet_feats, obj_pred, rel_pred = self.model(obj_pts, edge_indices.t().contiguous(), descriptor, batch_ids)
                ...
                tri_contrastive_loss = self.c_criterion(triplet_feats, pos_tri_pair, neg_tri_pair, rel_tri_indices)
                ...
                t_log = [
                    ...
                    ("train/pred_cont_loss", contrastive_loss.detach().item()),
    ...
    def evaluate_validation(self):
        ...
        with torch.no_grad():
            ...
            for i, (
                ...
                obj_pts = obj_pts.transpose(2, 1).contiguous()
                _, _, obj_pred, rel_pred = self.model(obj_pts, edge_indices.t().contiguous(), descriptor, batch_ids)
