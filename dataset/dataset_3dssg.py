...
class SSGLWBFeat3D(Dataset):
    ...
    def __get_data(
        ...
        for e in range(len(edge_indices)):
            ...
            edge_pts = points4d[np.where(filter_mask > 0)[0], :]
            choice = np.random.choice(len(edge_pts), 2 * num_pts_normalized, replace=True) # dim: (2 * N_pts) X self.dim_pts
            edge_pts = edge_pts[choice, :]
            edge_pts = torch.from_numpy(edge_pts.astype(np.float32))
            edge_pts[:,:3] = self.zero_mean(edge_pts[:,:3])
            # edge_pts = np.concatenate([obj_pts_1, obj_pts_2], axis=0) 
